...
const timeValidation = Joi.string()
  .custom((value, helpers) => {
    ...
    if (period === "AM" && (numericHour < 7 || numericHour >= 12)) {
      return helpers.error("any.invalid", {
        message: "Time must be between 7 AM and 5 PM",
      });
    }
  ...
  .messages({
    "any.invalid":
      "Invalid time format. Use HH:MM AM/PM format and make sure you remove the spaces",
  });
...
const generateMenuSchema = Joi.object().keys({
  ...
  initialQuantity: Joi.number().required().max(100),
