...
export let createGenerateMenu = tryCatchWrapper(async (req, res) => {
  ...
  let data = await generateMenuServices.createGenerateMenuService({
    body: body,
  });
  successResponseData({
    res,
...
export let updateGenerateMenu = tryCatchWrapper(async (req, res) => {
  ...
  let data = await generateMenuServices.updateSpecificGenerateMenuService({
    id,
    body,
  });
...
export let deleteSpecificGenerateMenu = tryCatchWrapper(async (req, res) => {
  let id = req.params.id;
  let data = await generateMenuServices.deleteSpecificGenerateMenuService({
    id,
  });
...
export let fetchFoodItems = tryCatchWrapper(async (req, res) => {
  try {
    const generateMenus =
      await generateMenuServices.readAllGenerateMenuService();
    // Populate the foodItem field with name and rate
    const populateMenus = await GenerateMenu.populate(generateMenus, {
      path: "foodItem",
      select: "name rate",
    });
